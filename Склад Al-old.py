...
def get_order_type():
    ...
    if selection == 1:
        main_dictionary['order_type'] = 'Продажба'
    elif selection == 0:
        main_dictionary['order_type'] = 'Каса'
    # elif selection == 2:
    #     main_dictionary['order_type'] = 'Покупка'
    return
...
#     cursor.execute("SELECT month, al_counter FROM orders")
#     record = cursor.fetchone()
#     month = record[0]
#     al_counter = record[1]
#     if current_month != month:
#         change_month = ("UPDATE orders SET month = %s, al_counter = %s")
#         new_month = (current_month, 1,)
#         cursor.execute(change_month, new_month)
#         al_counter = 1
#     connection.commit()
#     main_dictionary['production_order'] = (f"Al-{current_month}-{al_counter}")
#     return
...
def ok_button():
    ...
    if firm_cb.get() not in list_combobox():
        ...
        #     cursor.execute("SELECT al_counter FROM orders")
        #     al_counter = int(cursor.fetchone()[0])
        #     al_counter += 1
        #     insert_al_count = "UPDATE orders SET al_counter = %s"
        #     current_counter = (al_counter,)
        #     cursor.execute(insert_al_count, current_counter)
        connection.commit()
...
# bank_radio = ttk.Radiobutton(entry_window, text="Покупка", variable=radio_var, value=2, command=sel)
...
cursor.execute("SELECT partner.partner_name, records.order_type, records.ammount, records.note FROM records"
               " INNER JOIN partner ON records.partner_id = partner.partner_id"
               " WHERE warehouse = 'Управител' and date = current_date")
today_orders = cursor.fetchall()
for row in today_orders:
    tree_day_report.insert('', 'end', values=row)
    if row[1] == 'Каса':
        day_total_sum += int(row[2])
